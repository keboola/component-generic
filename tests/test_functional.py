...
from tests.functional.custom_matchers import (
    binary_payload_matcher,
    binary_gz_payload_matcher,
...
class TestComponent(unittest.TestCase):
    ...
    def test_binary_payload_iterations(self):
        ...
        expected_payloads = {"http://functional/test/123": content1, "http://functional/test/234": content2}

        responses.add(
            responses.POST,
            re.compile("http://functional/test/(123|234)"),
            match=[binary_payload_multi_matcher_to_string(expected_payloads), params_matcher],
        )
        comp.run()
    ...
    def test_binary_payload_full_table(self):
        ...
        responses.add(responses.POST, url="http://functional/test", match=[data_matcher, params_matcher])
    ...
    def test_log_error_response_content(self):
        ...
        responses.add(responses.POST, url="https://functional/test", body=response_text, status=400)
        try:
            comp.run()
        except UserException as e:
