...
class TestConfiguration(unittest.TestCase):
    ...
    def test_build_configuration_full(self):
        api = ApiConfig(
            ...
            default_query_parameters={"test": "test"},
            default_headers={"Authorization": {"attr": "token_encoded"}},
        ...
        user_parameters = {
            ...
            "token_encoded": {
                "function": "concat",
                "args": ["Basic ", {"function": "base64_encode", "args": [{"attr": "#token"}]}],
            },
        }
        ...
        content = RequestContent(
            ...
            json_mapping=JsonMapping(
                ...
                column_names_override={"column_a": "COLUMN|A"},
                column_data_types=ColumnDataTypes(autodetect=True, datatype_override=[{"column_a": "number"}]),
            ),
        )

        expected_cfg = WriterConfiguration(
            api=api, request_parameters=api_request, request_content=content, user_parameters=user_parameters
        )
        with open(os.path.join(self.resource_dir, "configv2.json")) as inp:
            config = json.load(inp)
    ...
    def test_build_configuration_minimal(self):
        ...
        api_request = ApiRequest(method="POST", endpoint_path="users/[[id]]", continue_on_failure=False)
        content = RequestContent(
            content_type="JSON",
            iterate_by_columns=["id", "date"],
            json_mapping=JsonMapping(
                chunk_size=1, nesting_delimiter="__", column_data_types=ColumnDataTypes(autodetect=True)
            ),
        )
